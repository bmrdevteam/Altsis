    ...
    for (const enrollment of enrollments) {
      ...
      if (idx === -1) {
        ...
        processedEvaluationByYear.push({
          ...
          ...enrollment._subject,
          ...enrollment._evaluation,
          [`${"연도별"}/학점[합산]`]: enrollment?.point || 0,
          [`${enrollment.term}/학점[합산]`]: enrollment?.point || 0,
        });
      } else {
        Object.assign(processedEvaluationByYear[idx], {
          [`${"연도별"}/학점[합산]`]:
            processedEvaluationByYear[idx][`${"연도별"}/학점[합산]`] +
              enrollment?.point || 0,
          [`${enrollment.term}/학점[합산]`]:
            (processedEvaluationByYear[idx][`${enrollment.term}/학점[합산]`] ||
              0) + (enrollment?.point || 0),
          ...enrollment?._evaluation,
        });
      }
    }
