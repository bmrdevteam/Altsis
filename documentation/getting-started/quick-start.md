...
MongoDB Insert는 Mongoose 훅을 거치지 않으므로, 먼저 bcrypt 해시를 만듭니다.
...
`root` 데이터베이스 > `users` 컬렉션에 다음 Document를 삽입합니다. `password`에는 출력된 해시를 넣습니다.
...
브라우저에서 아카데미 ID `root`로 입장한 뒤, `admin`과 해시 생성 시 사용한 비밀번호로 로그인합니다. 자세한 설명은 [초기 설정](initial-setup.md)을 참고하세요.

> [!WARNING]
> 인증 코드를 주석 처리하거나 비밀번호 검증을 우회하지 마세요.
