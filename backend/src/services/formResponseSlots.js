...
export const hasDocResponseSlots = (text) =>
  extractDocResponseSlots(text).length > 0;
 ...
 */
export const fillDocResponseSlotsInOrder = (template, values) => {
  const slots = extractDocResponseSlots(template);
  if (!slots.length) return String(template || "");
...
export const mergeDocResponseTemplate = (baseDocument, aiText) => {
  ...
  const slots = extractDocResponseSlots(base);
  if (!slots.length) {
  ...
  if (fills.length > 0) {
    ...
    return fillDocResponseSlotsInOrder(base, values);
  }
  ...
  if (preservesDocResponseSkeleton(base, cleaned)) {
    // data URI 양식은 AI 전체 교체를 허용하지 않음(깨진 로고 방지)
    if (/data:image\//i.test(base)) {
      const values = slots.map(() => null);
  ...
  // 양식을 버리고 새로 쓴 경우 → 본문 슬롯에만 넣고 골격 유지
  const values = slots.map(() => null);
  values[pickBodySlotIndex(slots)] = cleaned;
  return fillDocResponseSlotsInOrder(base, values);
};
...
/** 프롬프트용 슬롯 목록 요약 */
export const formatSlotsForPrompt = (baseDocument) => {
  const slots = extractDocResponseSlots(baseDocument);
