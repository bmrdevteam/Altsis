...
exports.register = async (req, res) => {
    try {
        ...
        if (yearIdx == -1) {
            ...
            const termIdx = schoolUser["registrations"][yearIdx][
                "terms"
            ].findIndex((obj) => obj === req.body.term);
            if (termIdx == -1) {
                schoolUser["registrations"][yearIdx]["terms"].push(
                    req.body.term
                );
            } else {
                return res
                    .status(409)
                    .send({
                        message: "schoolUser already registered in this term"
                    });
            }
        }
    ...
    try {
            ...
            req.body.schoolUsers.map(async (_schoolUser) => {
                ...
                if (!schoolUser) {
                    const error = new Error(
                        `no schooluser(_id:${_schoolUser._id}`
                    );
                    error.code = 404;
                    throw error;
                }
                ...
                const yearIdx = schoolUser["registrations"].findIndex(
                    (obj) => obj.year === _schoolUser.year
                );
                if (yearIdx == -1) {
                    schoolUser["registrations"].push({
                        year: _schoolUser.year,
                        terms: [_schoolUser.term],
                        info: _schoolUser.info
                    });
                } else {
                    const termIdx = schoolUser["registrations"][yearIdx][
                        "terms"
                    ].findIndex((obj) => obj === _schoolUser.term);
                    if (termIdx == -1) {
                        schoolUser["registrations"][yearIdx]["terms"].push(
                            _schoolUser.term
                        );
                        ...
                        const error = new Error(
                            "schoolUser already registered in this term"
                        );
                        error.code = 409;
                        throw error;
                    }
        ...
        await Promise.all(
            schoolUsers.map((schoolUser) => {
                schoolUser.save();
                console.log(
                    `debug: schoolUser.userId(${schoolUser.userId}) is saved`
                );
            })
        );
