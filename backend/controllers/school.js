...
exports.delete = async (req, res) => {
    try {
        const doc = await School(req.user.dbName).findByIdAndDelete(
            req.params._id
        );
...
exports.createField = async (req, res) => {
    try {
        ...
        if (_.isEmpty(req.body.new)) {
            return res
                .status(409)
                .send({ message: "body.new is null or empty!" });
        }
        ...
        if (req.params.field == "subjects") {
            ...
            if (
                req.params.field == "classrooms" &&
                _.indexOf(school["classrooms"], req.body.new) != -1
            ) {
                return res
                    .status(409)
                    .send({ message: "already existing classroom" });
            }
