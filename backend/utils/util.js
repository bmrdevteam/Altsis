...
exports.checkClassroomAvailable = async (syllabuses, syllabus) => {
    await Promise.all(
        syllabuses.map(s2 => {
            ...
            if (overlappedBlock) {
                const error = new Error(`classroom(${syllabus.classroom}) is unavailable at ${overlappedBlock.label}`);
                error.code = 409;
                throw error;
            }
        })
...
exports.checkTimeAvailable = async (enrollments, time) => {
    await Promise.all(
        enrollments.map(enrollment => {
            ...
            if (overlappedBlock) {
                const error = new Error(`user(${enrollment.userId},${enrollment.userName}) is unavailable at ${overlappedBlock.label}`);
                error.code = 409;
                throw error;
            }
        })
